...
*
* Temp (Degree Celcius)                        Magic Happens Here!
* 245-|                                               x  x
*     |                                            x        x
*     |                                         x              x
*     |                                      x                    x
...
*           - Oven temp might not reach configured maximum temp
*           - Adjusted values so learning happens faster
*           - Other minor improvements
* 1.2       Improvements (6 January 2014)
*           - Take temp readings every 0.125s and average them.  Fixes errors
...
*             "P" (proportional) of the PID algorithm.  Temps may overshoot
*             initially, but for the rest of the baking time (up to 18 hours) the
*             temp will be a few degrees below the target temp.
...
*           - Added Integral (the "I" of PID) to bake, so temps come closer
*             to desired bake temp
...
*           - Initial bake duty cycle is based on desired bake temp
*******************************************************************************/
...
void setup()
{
	...
	lcdPrintLineF(0, F("ControLeo2"), 3);
	lcdPrintLineF(1, F("Reflow Oven v1.9"));
	delay(100);
	Tunes::playStartup();
	delay(3000);
	...
	if ( Settings::get(Settings::LEARNING_MODE) == false )
		mode = 2;

	Serial.println(F("ControLeo2 Reflow Oven controller v1.9"));

	// Make sure the oven door is closed
	setServoPosition(Settings::get(Settings::SERVO_CLOSED_DEGREES), 1000);
}

#define NO_OF_MODES 10
#define NEXT_MODE false

// Main menu options
bool (*action[NO_OF_MODES])() = {Testing
								, Config
								, Reflow
								, Bake
								, RefreshPla
								...
								, DryNylon
								, DryDesiccant};
const char *modes[NO_OF_MODES] = {"Test Outputs?"
								, "Setup?"
								, "Start Reflow?"
								, "Start Baking?"
								, "Refresh PLA?"
...
void loop()
{
	...
	if ( showMainMenu )
	{
		if ( drawMenu )
		{
			...
			lcdPrintLineF(1, F("Yes ->"), 10);
		}

		// Update the temp roughtly once per second
		if ( ! (counter++ % 20) )
			displayTemp();
...
void lcdPrintLineF(int line, const __FlashStringHelper *fstr, int leadingSpaces)
{
	const char *str((const char *)fstr);
	int len(strlen_P(str));
	char buf[17] = "                ";

	if ( len > (16 - leadingSpaces) )
		len = 16 - leadingSpaces;

	for ( int i = 0; i < len; ++i )
		buf[leadingSpaces+i] = pgm_read_byte_near(str + i);

	lcd.setCursor(0, line);
	lcd.print(buf);
}

// Displays in the bottom left corner of the LCD display
void displayTemp(void)
{
	lcd.setCursor(0, 1);

	double temp(0.0);
	int fault(getCurrentTemp(temp));
